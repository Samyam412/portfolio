...
const LargeDock = ({
    ...
    return (
        <motion.div
            ...
            className={cn(
                "mx-auto hidden h-16 items-end gap-4 rounded-2xl px-4 pb-3 dark:bg-primary md:flex", // Large dock styles
                className,
        ...
        >
            ...
            {items.map((item) => (
                <DockIcon mouseX={mouseXPosition} key={item.title} title={item.title} icon={item.icon} href={item.href} />
            ))}
        </motion.div>
    );
};
                        ...
                        {items.map((item, index) => (
                            ...
                            >
                                <Link
                                    href={item.href ?? ""}
                                    key={item.title}
                                    className="flex h-10 w-10 items-center justify-center rounded-full text-primary shadow-md backdrop-blur-md dark:text-white"
                                >
